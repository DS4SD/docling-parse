...
from docling_core.types.doc.base import (ImageRefMode, CoordOrigin, Size, BoundingBox)
    ...
    def to_bottom_left_origin(self, page_height: float) -> "BoundingRectangle":
        ...
        if self.coord_origin == CoordOrigin.BOTTOMLEFT:
            ...
            return BoundingRectangle(
                ...
                r_y0 = page_height-r_y0,
                r_y1 = page_height-r_y1,
                r_y2 = page_height-r_y2,
                r_y3 = page_height-r_y3,
...
class SegmentedPage(BaseModel):
    ...
    def render(self,
               ...
               cropbox_width: int = 3,
               cropbox_alpha: float = 1.0,) -> PILImage.Image:
        ...
        def _get_rgba(name: str, alpha:float):
            ...
            rgba = ImageColor.getrgb(cell_bl_outline) + (
        ...
        if draw_cells_bbox or draw_cells_text:
            ...
            for page_cell in self.cells:
                ...
                if draw_cells_bbox:
                    ...
                    draw.polygon(rect, outline=outline, fill=fill)
                
                if draw_cell_bl:
                    _draw_circle(
                        draw,
                        point=rect[0],
                        radius=cell_bl_radius,
...
def _to_dimension(dimension: dict) -> PageDimension:
    ...
    return PageDimension(angle = dimension["angle"],
...
def _to_segmented_page(page: dict) -> SegmentedPage:
    ...
    return SegmentedPage(dimension = _to_dimension(page["dimension"]),
...
def _to_parsed_page(page: dict) -> ParsedPage:
    ...
    paginated_page = ParsedPage(original = _to_segmented_page(page["original"]),
                                sanitized = _to_segmented_page(page["sanitized"]))
    
    return paginated_page
    
def from_pdf_parser_v2_to_parsed_paginated_document(doc_dict: dict) -> ParsedPaginatedDocument:
