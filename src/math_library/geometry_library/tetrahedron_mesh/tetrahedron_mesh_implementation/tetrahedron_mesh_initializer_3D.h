...
namespace MATH_ALGORITHMS
{
  ...
  void tetrahedron_mesh_initializer<3, k_cluster_type>::make_convex_hull()
  {
    ...
    func_lib::function<double, func_lib::matrix_dmn_type> A("A", 3, 3);
    func_lib::function<double, func_lib::vector_dmn_type> B("X", 3);

    for(size_t l0=0; l0<B_collection.size(); l0++){
      for(size_t l1=0; l1<B_collection.size(); l1++){
	for(size_t l2=0; l2<B_collection.size(); l2++){

	  A(0,0) = B_collection[l0][0]; A(0,1) = B_collection[l0][1]; A(0,2) = B_collection[l0][2];
	  A(1,0) = B_collection[l1][0]; A(1,1) = B_collection[l1][1]; A(1,2) = B_collection[l1][2];
	  A(2,0) = B_collection[l2][0]; A(2,1) = B_collection[l2][1]; A(2,2) = B_collection[l2][2];
...

	  {
	    ...
	    lin_ops::lapack_operation<lin_ops::GESV>::execute(A, B);

	    double det_A = A(0,0)*A(1,1)*A(2,2);//slv_pln.matrix[0]*slv_pln.matrix[4]*slv_pln.matrix[8];
		...
		s.k_vec[0] = B(0);//slv_pln.solved_matrix[0];
		s.k_vec[1] = B(1);//slv_pln.solved_matrix[1];
		s.k_vec[2] = B(2);//slv_pln.solved_matrix[2];
