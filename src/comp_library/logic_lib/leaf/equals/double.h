...
#ifndef LOGIC_LIB_LEAF_EQUALS_DOUBLE
#define LOGIC_LIB_LEAF_EQUALS_DOUBLE

namespace logic_lib
{
  template<>
  class leaf<EQUALS, double> :
    public node<EQUALS>,
  ...
  leaf<EQUALS, double>::leaf(container_lib::container & logic_node) :
    node<EQUALS>(),
    leaf<UNKNOWN, double>(logic_node)
  {
  }

  std::string leaf<EQUALS, double>::repr() const
  {
    return "[" + field_string() + " <EQUALS> " + value_string() + "]";
  };

  bool leaf<EQUALS, double>::evaluate(container_lib::container & graph_node)
  {
    /*
