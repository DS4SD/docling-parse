...
#ifndef LOGIC_LIB_LEAF_CONTAINS_DOUBLE
#define LOGIC_LIB_LEAF_CONTAINS_DOUBLE

namespace logic_lib
{
  template<>
  class leaf<CONTAINS, double> :
  ...
  leaf<CONTAINS, double>::leaf(container_lib::container & logic_node) :
    node<CONTAINS>(),
    leaf<UNKNOWN, double>(logic_node)
  {
  }

  std::string leaf<CONTAINS, double>::repr() const
  {
    return "[" + field_string() + " <CONTAINS> " + value_string() + "]";
  };

  // FIXME: the get_size() is not used correctly!!
  bool leaf<CONTAINS, double>::evaluate(container_lib::container & graph_node)
  {
    /*
