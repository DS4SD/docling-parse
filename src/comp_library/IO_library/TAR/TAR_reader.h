...
int reader<TAR>::copy_data(struct archive *output)
{
  int r;
  const void *buff;
  std::size_t size;
...
template<>
char const * reader<TAR>::convert<std::string>::c_str(std::string const & s)
{
  return s.c_str();
}

template<>
char const * reader<TAR>::convert<std::string const>::c_str(std::string const & s)
{
  return s.c_str();
}
...
//-----------------------------------------------------------------------------
template<typename T>
int reader<TAR>::opencallback(struct archive *, void *_client_data)
{
  if (!_client_data)
    {
...
template<typename T>
int reader<TAR>::closecallback(struct archive *, void *_client_data)
{
  if (!_client_data)
    {
      return ARCHIVE_FATAL;
...
template<typename T>
reader<TAR>::archive_ssize_type  reader<TAR>::readcallback(
  struct archive *,
  void *_client_data,
  const void **_buffer)
{
...
template<typename T>
reader<TAR>::archive_int64_type reader<TAR>::skipcallback(struct archive * input,
    void * _client_data,
...
typename std::enable_if < meta_lib::traits::like::stream<T>::value, void >::type
reader<TAR>::extract(T && in_stream_like, S && fspath)
{
...
//-----------------------------------------------------------------------------
template<typename T, typename S>
void reader<TAR>::execute(T && in_stream_like, S && fspath)
{
