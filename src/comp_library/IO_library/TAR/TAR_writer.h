...
//-----------------------------------------------------------------------------
writer<TAR>::writer(int iflags, bool const striproot)
  : flags(iflags),
...
template<>
char const * writer<TAR>::convert<std::string>::c_str(std::string const & s)
{
  return s.c_str();
}

template<>
char const * writer<TAR>::convert<std::string const>::c_str(std::string const & s)
{
  return s.c_str();
}

int writer<TAR>::read_data(struct archive * from)
{
  std::size_t size = 0;
...
int writer<TAR>::copy_data(struct archive *from)
{
...
//-----------------------------------------------------------------------------
template<typename T>
int writer<TAR>::opencallback(struct archive *, void *_client_data)
{
  if (!_client_data)
    {
...
template<typename T>
int writer<TAR>::closecallback(struct archive *, void *_client_data)
{
  if (!_client_data)
    {
      return ARCHIVE_FATAL;
...
template<typename T>
writer<TAR>::archive_ssize_type  writer<TAR>::writecallback(
  struct archive *,
  void *_client_data,
  const void *_buffer,
  size_t _length)
...
typename std::enable_if < meta_lib::traits::like::stream<T>::value, void >::type
writer<TAR>::create(T && oslike, S && fspath)
{
  T & stream = oslike;
...
//-----------------------------------------------------------------------------
template<typename T, typename S>
void writer<TAR>::execute(T && oslike, S && fspath, bool const striproot)
{
